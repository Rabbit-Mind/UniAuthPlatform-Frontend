...
withDefaults(defineProps<Props>(), {
  description:
    '是一个现代化开箱即用的多租户OAuth2认证平台前端解决方案，基于Vben Admin框架二次开发，采用最新的技术栈，包括 Vue 3.0、Vite、TailwindCSS 和 TypeScript 等前沿技术，代码规范严谨，提供丰富的配置选项，旨在为中大型项目的开发提供现成的开箱即用解决方案及丰富的示例，同时，它也是学习和深入前端技术的一个极佳示例。',
  name: '通用认证平台',
  title: '关于项目',
});
...
const renderLink = (href: string, text: string) =>
  h('a', { href, target: '_blank', class: 'ua-link' }, { default: () => text });
...
const BaseDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: 'Vben Admin 版本号',
  },
  {
    content: buildTime,
    title: '平台最后构建时间',
...
const LinkDescriptionItems: DescriptionItem[] = [
  {
    content: renderLink(homepage, '点击查看'),
    title: '模板主页',
  },
  {
    content: renderLink(RM_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(UA_GITHUB_URL, '点击查看'),
    title: '项目 Github',
  },
  {
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="UA_GITHUB_URL" class="ua-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box mt-6 p-5">
      <div>
        <h5 class="text-foreground text-lg">链接信息</h5>
      </div>
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in LinkDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              <dt class="text-foreground text-sm font-medium leading-6">
                {{ item.title }}
              </dt>
